...
/**
 * Provides mapping from a HTTP-specific request to a protocol-independent
 * request.
 * 
 * @author <ul>
...
public class RestHttpServlet extends HttpServlet {
	...
	/**
	 * Converts a {@link HttpServletRequest} to a {@link RequestIndication} and
	 * uses it to invoke the SCL service. Converts the received
	 * {@link ResponseConfirm} to a {@link HttpServletResponse} and returns it
	 * back.
	 */
	@Override
	protected void service(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
		// XXX junhao
		System.out.println("In RestHttpServlet.java");
		System.out.println("Protocol = " + httpServletRequest.getProtocol()); // HTTP/1.1
		System.out.println("Header(Authorization) = " + httpServletRequest.getHeader("Authorization"));
		System.out.println("Method = " + httpServletRequest.getMethod()); // GET
		System.out.println("QueryString = " + httpServletRequest.getQueryString()); // null
		System.out.println("ServerName = " + httpServletRequest.getServerName()); // localhost
		...
		System.out.println("ServletPath = " + httpServletRequest.getServletPath()); // /om2m
		System.out.println("PathInfo = " + httpServletRequest.getPathInfo());
		System.out.println("AuthType = " + httpServletRequest.getAuthType()); // null
		System.out.println("ServerPort = " + httpServletRequest.getServerPort()); // 8080
...
//		if (Constants.SCL_TYPE.equalsIgnoreCase("gscl") && httpServletRequest.getRemoteAddr().equalsIgnoreCase(Constants.NSCL_IP)) {
//
//			test(httpServletRequest, body);
//
//		} else {
//			PrintWriter out = null;
//			try {
...
//		}
	...
	private void test(HttpServletRequest httpServletRequest, String body) {
...
//		}
	...
	/**
	 * Converts a standard HTTP query String into a protocol independent
	 * parameters map.
	 * 
	 * @param query
	 *            - standard HTTP query String.
