...
//	public static int cnt=0;
	...
	// 500s => change 1000 times 
	public static int period=600;
	
	public static double loss_rate=0.0;
...
//	}
	...
	public static void startchange() {
		...
		for (int i = 1; i <= period*5; ++i) {
			...
			if(i%period>0 && i%period<100) {
				loss_rate+=((double)15/100);
			}
			if(i%period>=100 && i%period<200) {
				loss_rate-=((double)15/100);
			}
			if(i%period>=200 && i%period<400) {
				if(i%period==200) {
					bandwidth=100;
				}
				loss_rate+=((double)20/200);
			}
			if(i%period>=400 && i%period<500) {
				if(i%period==400) {
					bandwidth=500;
				}
				loss_rate+=((double)5/100);
			}
			if(i%period>=500 && i%period<600) {
				loss_rate-=((double)25/100);
			}
			
			change(loss_rate,bandwidth);
	...
	public static int change(Double iPacketLossRate,Integer bandwidth) {
		String strPacketLossRate = iPacketLossRate.toString();
		String applied_bandwidth= bandwidth.toString();
		...
		try {
			...
			body[3] = new NameValuePair("txtBandwidth1", applied_bandwidth);
