...
public class writePacketLossRateTaskLoop extends TimerTask {
	...
	public static void setProcess(Process process) {
		writePacketLossRateTaskLoop.process = process;
	}
	...
	/** Logger */
	private static Log LOGGER = LogFactory.getLog(writePacketLossRateTaskLoop.class);
	...
	private String strDefaultPacketLossRate = "";
	private String strDefaultDataSize = "";
	private String strLoopNum = "";
	...
	public writePacketLossRateTaskLoop() {

	}

	public writePacketLossRateTaskLoop(String strDefaultPacketLossRate, String strDefaultDataSize, String strLoopNum) {
		this.strDefaultPacketLossRate = strDefaultPacketLossRate;
		this.strDefaultDataSize = strDefaultDataSize;
		this.strLoopNum = strLoopNum;
	}

	public static void setScl(SclService scl) {
		writePacketLossRateTaskLoop.SCL = scl;
	}
	...
	public void run() {
		...
		try {
			...
			String strExtendFileName = "";

			if (strDefaultPacketLossRate.length() > 0 && strDefaultDataSize.length() > 0 && strLoopNum.length() > 0) {
				strExtendFileName = "_" + strDefaultPacketLossRate + "%_" + strDefaultDataSize + "_" + strLoopNum + ".txt";
				strFileName = "test_pingResult_" + pingIP + ".txt" + strExtendFileName;
			}
			...
			// 計算行數
			int iLineNumber = calculateLineNumber(strFileName);
			System.out.println("iLineNumber = " + iLineNumber);

			// 計算ping成功的有幾行
			int iTTLNumber = calculateTTLNumber(strFileName);
			...
			if (strPacketLossRate != null) {
				...
				writeFile(strPacketLossRate, "test_packetlossrate_" + pingIP + ".txt" + strExtendFileName, true);
			}
	...
	private int calculateLineNumber(String strFileName) {
		int iLineNumber = -1;
